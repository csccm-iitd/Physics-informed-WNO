...
class gradientfree(nn.Module):
    ...
    def neighbour_index(self,X_f_train):
        ...
        for j in range(X_f_train.shape[0]): 
            ...
            x = X_f_train[j,:][None,:]
            n = x.shape[1]
            ngr1_d= torch.where(torch.linalg.norm(X_f_train-x,axis=1)<radius)[0].tolist()
    ...
    def loss_PDE(self,s_xy,x_to_train_f,u,n_index,inv_mat):
    ...
    def loss(self,s_xy,up,usol,x_to_train_f,u,n_index,inv_mat):

        loss_u = self.loss_BC(up,usol)
        loss_f = self.loss_PDE(s_xy,x_to_train_f,u,n_index,inv_mat)
