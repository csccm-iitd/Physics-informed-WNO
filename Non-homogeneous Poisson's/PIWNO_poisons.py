...
reader = scipy.io.loadmat('u_sol_poissons.mat')  	# Load data from file
x = torch.tensor(reader['x1d'],dtype = torch.float)                                   # 200 points between -1 and 1 [256x1]
y = torch.tensor(reader['y1d'],dtype = torch.float)                                   # 200 time points between 0 and 1 [100x1] 
...
x_train = x_train1[:ntrain,:,:]
y_train = y_train1[:ntrain,:,:]

x_test = x_train1[ntrain:ntrain+ntest,:,:]
y_test = y_train1[ntrain:ntrain+ntest,:,:]
...
for ep in range(epochs):
    ...
    for x, y in train_loader1:
        ...
        x = x_normalizer.decode(x.squeeze(3))
        ...
        for kk in range(out.shape[0]):
            ...
            physics_loss += gf.loss(x_pf,all_u_train,all_u_sol,x_f_train,y_pf,p_index,invp_index)
...
# torch.save(model, 'model/model_phy_cwno_poissons1')
scipy.io.savemat('pred/prediction_pinn_wno_poissons1.mat', mdict={'pred': pred.cpu().numpy()})

scipy.io.savemat('epoch_loss/train_loss_poissons1.mat', mdict={'train_loss': train_loss.cpu().numpy()})
scipy.io.savemat('epoch_loss/pred_loss_poissons1.mat', mdict= {'test_loss': test_loss.cpu().numpy()})
